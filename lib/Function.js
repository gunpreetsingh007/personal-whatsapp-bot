...
exports.getRandomWithoutExtension = ( length = "10") => {
    var result = ""
    var character = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890"
    var characterLength = character.length
    for (var i = 0; i < length; i++) {
...
exports.GIFBufferToVideoBuffer = async (image) => {
    const filename = this.getRandomWithoutExtension();
    fs.writeFileSync(`temp/${filename}.gif`, image, "binary")
    //ini_buff = fs.readFileSync(file_name)
    let child = execSync(
      `ffmpeg -f gif -i temp/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" temp/${filename}.mp4`
    );
    const buffer = fs.readFileSync(`temp/${filename}.mp4`);
    fs.unlinkSync(`temp/${filename}.mp4`)
    fs.unlinkSync(`temp/${filename}.gif`)
    return buffer;
  };
